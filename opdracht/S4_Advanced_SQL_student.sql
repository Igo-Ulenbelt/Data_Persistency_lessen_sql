...
SELECT mnr, functie, gbdatum FROM medewerkers WHERE gbdatum < '1980-01-01'AND (functie = 'VERKOPER' OR functie = 'TRAINER');
...
SELECT CONCAT(voorl, '. ', naam) AS volledige_naam FROM medewerkers;
...
SELECT u.cursus, u.begindatum, COUNT(i.cursist) AS aantal_inschrijvingen FROM uitvoeringen AS u
INNER JOIN inschrijvingen AS i ON u.cursus = i.cursus AND u.begindatum = i.begindatum
WHERE u.begindatum BETWEEN '2019-01-01' AND '2020-01-01' GROUP BY u.cursus, u.begindatum HAVING COUNT(i.cursist) > 2;
...
SELECT m.mnr, i.cursus, COUNT(i.cursist) as agevolgd FROM medewerkers as m
INNER JOIN inschrijvingen as i ON m.mnr = i.cursist
GROUP BY m.mnr, i.cursus HAVING COUNT(i.cursist) > 1
...
SELECT c.code, COUNT(u.cursus) as aantal FROM cursussen as c
INNER JOIN uitvoeringen as u ON c.code = u.cursus GROUP BY c.code
...
SELECT ROUND(MAX(EXTRACT(YEAR FROM AGE(gbdatum))) - MIN(EXTRACT(YEAR FROM AGE(gbdatum)))) AS verschil, ROUND(AVG(EXTRACT(YEAR FROM AGE(gbdatum))), 2) AS gemiddeld FROM medewerkers;
...
SELECT
    ...
    ROUND(AVG(comm), 2) AS commissie_medewerkers,
    ROUND(AVG(CASE WHEN functie = 'VERKOPER' THEN comm ELSE NULL END), 2) AS commissie_verkopers
FROM medewerkers;
